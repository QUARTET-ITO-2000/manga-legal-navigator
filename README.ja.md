...
* ボタンは各ストアの結果を反映するようになりました: **結果があったストアはハイライトされ前に出ます**（DLsite が先頭、以降は設定順）。結果が無かったストアはグレーのまま後ろに並びます。どこにも無かった場合は**作者リンクがハイライト**されます（この時点で唯一役に立つ操作のため）。
* Pixiv の作者リンクはサイト自身が使うパラメータに変更: `/search/users?nick=<名前>&s_mode=s_usr`（以前の `?word=` では誰も見つかりませんでした）。
* 作者名は**日本語**を優先するようにしました。日本語タイトルの角括弧内の表記（`[らーめん] …`）を使います。情報ブロックの `Artists` / `Groups` はローマ字スラッグ（`ra-men 171`）で、それでは日本語サイトで検索できないためです。
